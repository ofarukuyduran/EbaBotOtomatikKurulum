...
Write-Host "Desteklenen isletim sistemleri:
      ...
      Windows 7 (32bit/64bit)" -ForegroundColor Yellow
...
#New-Item -ItemType directory -Path $Name
 #   Write-Host "$Name Dizini oluşturuldu."
  ##   Invoke-WebRequest https://www.python.org/ftp/python/3.9.1/python-3.9.1-amd64.exe -OutFile C:\tmp\python-3.9.1-amd64.exe
}
}
...
if (Check_Program_Installed "Python"){
    ...
    if (-not (Test-Path "C:\tmp\")) {
        ...
        #Invoke-WebRequest https://www.python.org/ftp/python/3.9.1/python-3.9.1-amd64.exe -OutFile C:\tmp\python-3.9.1-amd64.exe
        downloadFile "https://www.python.org/ftp/python/3.9.1/python-3.9.1-amd64.exe" "C:\tmp\python-3.9.1-amd64.exe"
        Write-Host "Python İndirildi"  -ForegroundColor Green

        if( Test-Path "C:\tmp\python-3.9.1-amd64.exe"){
        ...
        $process = Start-Process -FilePath "c:\tmp\python-3.9.1-amd64.exe" -ArgumentList "/quiet InstallAllUsers=1 PrependPath=1" -wait
